# cc-harness · 재사용 가능한 Codex·Claude Code 개발 하네스

cc-harness는 AI와 함께 소프트웨어를 개발할 때 작업 순서, 역할, 승인 시점, 검증 기준을 안내하는 도구 모음입니다. 아이디어를 요구사항, 설계, 이슈별 구현·검증, 운영 인계까지 연결하는 **15개 프로젝트 스킬과 실무 방법론**으로 구성됩니다. 여기서 스킬은 일을 진행하는 절차를 뜻합니다. Codex는 `.agents/skills`와 `AGENTS.md`를, Claude Code는 같은 원본을 가리키는 `.claude/skills` 진입점과 `CLAUDE.md`를 읽고, 역할별 작업은 Opus 5.5 기준의 `.claude/agents` 서브에이전트에 맡깁니다. 앱, 독립 에이전트, 지속적 통합(CI) 환경까지 자동으로 설치한다는 의미는 아닙니다.
...
Claude Code를 쓴다면 새 제품 루트에 `@AGENTS.md`를 불러오는 `CLAUDE.md`를 만들고 `claude`를 실행한 뒤, 위 입력의 `$harness-cycle`을 `/harness-cycle`로 바꿉니다. 자세한 내용은 [새 PC·프로젝트 이식](docs/harness/reuse.md)의 `Claude Code를 쓸 때`를 참조합니다.
...
Git 훅, Codex·Claude Code 훅, 자동 에이전트, 제품 검사, 원격 CI, 사용자 수락, 배포는 서로 다른 검증 대상입니다. 새 PC에서는 설치 버전, 연결 상태, 신뢰 승인, 권한을 각각 확인해야 합니다.
