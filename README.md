...
cc-harness는 AI와 함께 소프트웨어를 개발할 때 작업 순서, 역할, 승인 시점, 검증 기준을 안내하는 도구 모음입니다. 아이디어를 요구사항, 설계, 이슈별 구현·검증, 운영 인계까지 연결하는 **15개 프로젝트 스킬과 실무 방법론**으로 구성됩니다. 여기서 스킬은 일을 진행하는 절차를 뜻합니다. Codex는 `.agents/skills`와 `AGENTS.md`를, Claude Code는 같은 원본을 가리키는 `.claude/skills` 진입점과 `CLAUDE.md`를 읽습니다. 앱, 독립 에이전트, 지속적 통합(CI) 환경까지 자동으로 설치한다는 의미는 아닙니다.
