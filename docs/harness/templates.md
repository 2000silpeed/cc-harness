...
기술 결정은 처음에는 미정으로 둡니다. 실제 구조에 맞는 대안을 최소 세 가지 비교한 뒤 사용자가 선택하면, 기술 결정 기록(ADR)의 네 요소를 작성합니다. 이어서 하지 않을 일을 구체화하고 승인을 받습니다. 의미 없는 대안을 끼워 넣어 특정 선택을 유도해서는 안 됩니다. 제품 요구사항 문서(PRD)는 결정의 기준점이며 디자인 값을 중복 저장하는 문서가 아닙니다.
