...
새 대상의 `docs/features/{feature}/`에 필요한 템플릿만 사용한다. 빈 문서를 대량 생성하지 않는다. 대괄호는 작성 시 교체하고 확정하지 못한 항목은 미정 이유·결정자를 적는다. 대상 프로젝트의 기존 인터뷰에 이 템플릿을 덮어쓰지 않는다.
...
한 이슈만으로 사용자 동작을 보여줄 수 있는지 검토한다. API→상태→UI만으로 수평 분할하지 않는다. 반나절~하루는 이슈 크기 추정 기준이며 실제 작업 시간 보장은 아니다. GitHub는 이슈 목록 승인 후 대상 저장소/보드/권한을 확인해 등록한다. 일부 성공 시 이미 생성한 URL을 기록해 중복 재등록하지 않는다.

## 테스트·검증

[TDD 방법론](../methods/tdd.md)의 시나리오·AC·보안 템플릿을 이슈별 `tests/`와 `reviews/`에 사용한다. 요구사항 ID → AC → 시나리오 ID → 테스트 코드 → 실행 증거의 연결이 끊기지 않아야 한다.
