...
G1–G5는 필요한 결정과 증거를 확인하는 지점입니다. 구현 승인이 없는 기본 모드는 `human-required`이며 각 사용자 결정을 기다립니다. 다만 사용자가 명확한 추천 결과에 “승인하고 구현”처럼 답했다면, 문맥상 그 추천 결과의 채택과 이를 완성할 가역적인 로컬 구현·검증을 승인한 것입니다. 선택 번호나 G1–G5 이름, 별도 위임 양식을 다시 요구하지 않습니다. 추천안이 여러 개라 지시 대상이 불분명하거나 목적·가치·중대한 범위·위험이 해결되지 않았다면 관련 질문 하나를 먼저 합니다.

승인된 결과 안에서 통상적인 기술안, 상세 디자인, 포함·제외 경계의 구체화, 이슈 분해와 테스트 계약은 AI가 판단하고 계속 진행합니다. G2·G3은 `policy-delegated`, G4·G5는 분리된 검증자의 검토를 거쳐 `auto-with-audit`로 기록할 수 있습니다. 사용자 원문·대상 결과·실행 승인과 AI가 선택한 기본값·이유·적용 범위를 분리하며 AI 선택을 사용자가 직접 골랐다고 쓰지 않습니다. 이 승인은 산출물의 자동 완료가 아닙니다. G1 spec과 필요한 design, G2·G3 PRD/ADR, G4 AC·의존성, G5 계약·시나리오, 실제 Red/Green과 독립 AC 검증의 근거를 차례로 갖춰야 합니다.

승인된 목적을 바꾸지 않는 낮은 위험의 미정 세부는 작은 가역적 기본값과 이유·변경 시 영향을 명시해 해결합니다. 사용자 결정이 필요한 목적·가치·위험·범위의 공백을 기본값으로 메우거나, 필수 동작 정책을 단순히 “미결”로 남긴 채 G1 통과로 기록하지 않습니다. 이미 유효한 동일 범위의 승인은 다시 묻지 않습니다. “G5까지만, 구현 금지” 같은 구체적 제한, 명시된 예산과 만료·철회, 객관 STOP은 포괄적인 “계속”보다 우선합니다. 사용자가 이후 “이제 구현”처럼 해당 제한을 명확히 바꿨다면 그 원문과 변경 범위를 기록해 반영합니다. 새 범위나 중대한 기술·비용·데이터·보안 영향은 해당 결정으로 돌아가 확인합니다.

별도로 이름 붙인 게이트 정책을 위임받았다면 대상/버전, 목적·판단 기준, 허용·금지 범위, 비용·시간·위험 한도, 외부 권한, 만료·철회, 상향·STOP을 기록하고 그 한계 안에서만 판정합니다. 이런 특수 정책의 누락·충돌을 통상적인 로컬 구현 승인으로 우회하지 않습니다. 일상적인 구현 승인에는 사용자가 말하지 않은 횟수나 기한을 만들어 필수 입력으로 요구하지 않습니다.

일반 구현 승인은 `tdd-auto-loop`의 무제한 자동 반복 위임이 아닙니다. 기본 `tdd-loop`에서 유한한 작업 단위와 의존 순서, 실제 시도·결과를 기록하며 계속합니다. 자동 반복을 별도로 요청하면 기존 이슈·재시도 상한과 STOP 계약을 적용합니다. 막힌 원인이 범위 내 국소 구현 오류라면 복구하고, 중대한 미결 결정·권한 부족·해소되지 않는 환경 문제·진전 없는 반복은 근거와 재개 조건을 남깁니다.

새 외부 사이트 생성, 계정 연결, Git 초기화·브랜치·커밋·푸시·PR·머지, 외부 게시·배포, 유료 호출, 파괴적 데이터 작업, 보안 위험을 바꾸는 조치에는 정확한 대상과 범위의 별도 권한이 필요합니다. 구현 승인만으로 이 권한을 추정하지 않습니다. 승인한 로컬 구현에 통상적으로 필요한 문서 조회·의존성 다운로드는 목적과 경계를 기록해 수행할 수 있지만, 외부 서비스 쓰기·비밀 전송·과금까지 포함하지 않습니다. 기존 네트워크 제한과 실제 도구 권한도 지킵니다.

## 6. 이슈 한 개의 운영 루프

```text
선행 이슈·승인·대상 브랜치 확인
  → 계약·시나리오 → [사용자 검토 또는 구현 승인 범위의 독립 검토]
  → Red(실패 원인 확인)
  → Green(최소 구현·회귀 확인)
  → 독립 AC 검증
       누락이면 시나리오·Red로 돌아감
...
반복 모드는 `tdd-loop`(단계별 검토와 §5의 유효한 구현 승인 적용)와 `tdd-auto-loop`(격리된 단계 위임·JSON 증거·위임된 기술 판단·객관 STOP) 중 선택합니다. 자동 모드도 한 이슈에 적용할 수 있으며 여러 이슈는 명시된 집합과 유한 상한 안에서만 반복합니다. 기획의 사용자 결정을 건너뛰지 않습니다. 승인 부재·Broken 환경·진전 없는 반복·새 범위·보안 차단·충돌·상한 도달 시 기록 후 정지합니다.
