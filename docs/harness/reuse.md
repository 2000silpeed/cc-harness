...
**cc-harness는 재사용 원본으로 두고, 실제 제품은 별도 형제 폴더에서 개발한다.** 전체 저장소에서 PM 앱을 지우는 대신 설치 스크립트로 등록된 스킬·방법론만 가져온다.
...
my-new-project는 실제 제품 이름으로 바꾼다. **새 제품을 cc-harness 안에 만들지 않는다.** 기존 PM 지침·Git 이력·검사 범위가 섞이는 것을 피하기 위해서다.
...
**복사·등록 검사에는 원본의 npm ci나 PM 앱 실행이 필요 없다.** 두 스크립트는 Node 기본 모듈을 사용한다. 새 폴더에는 아직 package.json이 없으므로 npm run dev도 실행할 수 없다. npm ci는 npm 기반 제품 구성과 잠금 파일이 생긴 뒤 해당 제품 폴더에서 실행한다.
...
| docs/harness·docs/methods 등록 문서 | PM src·tests·인터뷰·디자인·ontology        |
...
PM 사례의 요구사항·디자인·승인·기술안을 내 결정으로 복사하지 마.
...
AGENTS의 최소 예시다. 아직 없는 검사 명령을 작동한다고 기록하지 않는다.
...
- 제품 디자인·기술안은 별도 확정한다. PM 사례를 결정으로 복사하지 않는다.
...
스킬은 위임 런타임을 설치하지 않는다. 도구가 없다면 자동 모드 준비 미완료로 보고하고 단계별 방식으로 전환 여부를 결정한다. 테스트 통과와 AC 충족은 다르다. STOP이면 원인·재개 조건을 남기고 입력 보강 후 재개한다.
...
PM 전용 색상·검사기·절대 경로를 복사하지 마.
```

Git 훅은 Git 시점, Codex 훅은 지원되는 도구 이벤트 시점에 실행된다. 원본 hooks.json은 구조도·PM 경로와 셸 문법에 묶여 있어 이식하지 않는다. 버전·Windows 셸·신뢰를 새 환경에서 검증한다. 원본 npm run check도 PM 앱까지 포함하므로 새 제품은 자체 검사 묶음을 만든다.
...
- 원본을 pull했다고 제품의 독립 사본이 자동 변경되지 않는다.
...
| 현재 PM 앱을 다른 PC에서 실행   | cc-harness의 커밋된 앱 범위와 PM 기록을 확인하고 원본 README 실행 절차 사용                         |
| PM 업무 데이터 이동             | 전체 백업 JSON을 별도로 안전하게 전달하고 대상 데이터 확인 후 복원. 전체 교체이므로 주의            |

IndexedDB·tmp 샘플·미커밋 소스·로컬 대화·Codex 인증·.env·서버는 clone으로 옮겨지지 않는다. PM 데이터는 PC·브라우저·origin(프로토콜/호스트/포트)에 묶인다. 같은 PC라도 5173과 5174는 별개다. 새 제품에는 기존 PM 데이터를 가져오지 않는다.
