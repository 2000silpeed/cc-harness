...
- 이 저장소의 구조도 색상·PM 사례를 새 제품의 기준으로 복사하지 않는다. 현재 검사기는 구조도 HTML용이며 다른 경로를 인자로 주어도 규칙 원본은 바뀌지 않는다. 실제 이식에는 기준 원본·대상·스타일 방식·명령을 함께 조정한다.
- README와 흐름 화면의 상태를 함께 갱신한다. 문서 작성·사용자 승인·구현·수동 검사·훅 실제 발화를 구분해서 보고한다.

## 현재 저장소의 기준을 적용할 때

1. 프로젝트 AGENTS.md와 변경 대상의 실제 코드를 읽는다. 기존 PM 인터뷰와 승인 상태는 요청 없이 변경하지 않는다.
