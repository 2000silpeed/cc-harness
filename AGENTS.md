...
- 실제 프로젝트 진행에 사용하는 독립형 Codex 하네스다. 실행 스킬, 작성 방법론, 구조도와 코드 품질 도구가 있다.
- 제품 앱은 포함하지 않는다. Vitest의 TypeScript 하네스 이식 회귀 검사와 타입 검사를 유지한다. 원격 CI는 없다.
- 사람용 설명서와 검증 기록은 `README.md`, 구조도는 `docs/architecture/index.html`이다.
- 미사용 원본·호환 파일·임시 자료를 다시 가져오거나 Git에 포함하지 않는다.
- 전체 사이클 진행·재개에는 `docs/harness/lifecycle.md`, 스킬 등록·이식에는 `docs/harness/registry.json`을 확인한다. 요구사항·아키텍처·범위·이슈·시나리오 승인 근거를 보존한다. 등록과 실제 제품 실행을 구분한다.
...
- 시작할 때 요청한 결과와 완료에 필요한 검증을 확인한다. 승인된 범위의 가역 수정·검증·실패 복구는 재승인 없이 끝까지 진행한다. 미승인 결정 게이트와 STOP 조건은 유지한다. 별도 요청 없이 커밋·푸시·브랜치 생성·전역 설정 변경을 하지 않는다.
...
스킬 설명은 사용 조건 중심으로 짧게, 상세 참조는 해당 단계에서 읽도록 둔다. 근거: [OpenAI의 Astra 지침](https://developers.openai.com/blog/rethinking-skills-and-prompts-for-gpt-6-astra). 아래 역할 분리와 승인·검증 계약은 이 하네스의 명시적 요구사항이다.

## 필수 역할 분리

- 대화형·자동 실행과 하네스 자체 유지보수 모두 [공통 실행 정책](docs/methods/delivery-automation.md#필수-오케스트레이션-정책)을 적용한다. 메인은 상태·승인·라우팅·증거 통합·사용자 대화와 진행 안내를 맡고, 실제 탐색·산출물 작성/수정·테스트 실행은 서브에이전트에 위임한다. 작은 구현도 예외가 없다.
- 작업자는 받은 범위를 직접 수행하고 재귀 위임하지 않는다. 메인은 최소 입력의 새 작업 컨텍스트와 별도 검증자를 배정하며, 도구가 없으면 정확한 장애·재개 조건을 알리고 직접 구현으로 대체하지 않는다. 승인 게이트는 유지한다.
...
- 최종 통합 검사: `npm run check`. ESLint·디자인 정적 검사·하네스·Prettier·이식 회귀 검사·타입 검사를 실행한다. 앱 빌드·브라우저 검사·Windows 실기능 검증·원격 CI를 뜻하지 않는다.
- 관련 검사 → 실패 원인 수정·해당 검사 재실행 → 최종 `npm run check` 1회를 기본으로 한다. 새 변경·실패·필수 검사 없이 반복하지 않는다.
...
- 루트 `docs/design-system/`은 구조도용이다. `npm run design:check`는 구조도 정적 검사이며 디자인 승인이나 실제 화면 검증을 뜻하지 않는다. 새 제품은 대상의 지침과 디자인 기준을 별도로 준비한다.
